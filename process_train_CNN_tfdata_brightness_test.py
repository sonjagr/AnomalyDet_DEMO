...
from helpers.cnn_helpers import crop, patch_images,patch_labels, rotate_image, ds_length, grad, tf_bright_image, tf_bayer2rgb, tf_rgb2bayer, bright_image
...
def preprocess(imgs, lbls, to_encode = True, normal_times = 50, to_augment = False, to_brightness = False, to_brightness_before_ae = False, batch_size = 256):
    ...
    if to_encode == True:
        ...
        if to_brightness_before_ae:
            bright_diff_imgs = bright_cropped_imgs.map(encode)
    if to_encode == False:
    ...
    combined_dataset_batch = normal_dataset.concatenate(combined_anomalous_dataset).shuffle(buffer_size =  20000, reshuffle_each_iteration = True).batch(batch_size=batch_size, drop_remainder=True)
...
train_combined_dataset_batch, train_ano_before, train_ano = preprocess(train_imgs, train_lbls, to_encode = True, normal_times = 50, to_augment = True, to_brightness = True, to_brightness_before_ae=False, batch_size = batch_size)
