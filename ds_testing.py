...
X_train_det_list_orig = np.load(base_dir + dir_det + 'X_train_DET_very_cleaned.npy', allow_pickle=True)
X_val_det_list_orig = np.load(base_dir + dir_det + 'X_test_DET.npy', allow_pickle=True)
X_train_det_list = [images_dir_loc + s for s in X_train_det_list_orig]
X_val_det_list = [images_dir_loc + s for s in X_val_det_list_orig]
...
test_ds = create_cnn_dataset(X_val_det_list, Y_val_det_list, _shuffle=False)
plt.interactive(False)
train_ds = train_ds.map(process_crop, num_parallel_calls=tf.data.experimental.AUTOTUNE)
test_ds = test_ds.map(process_crop, num_parallel_calls=tf.data.experimental.AUTOTUNE)

count = 0
Y_train_det_list_new = []
X_train_det_list_new = []
from tqdm import tqdm
for x,y in tqdm(test_ds, total = 1600):
    ...
    for i in range(0, len(labels)):
        ...
        if label == 1.:
            ...
            plt.show()
            ip = input('Type r if you want to remove label on previous, press enter if ok.\n')
            if ip == 'r':
                labels[i] = 0.
    if np.isin([1.], labels):
        X_train_det_list_new.append(X_val_det_list_orig[count])
        Y_train_det_list_new.append(labels)
    count = count + 1

print(len(Y_train_det_list_new))
np.save(base_dir + dir_det + 'Y_test_DET_very_cleaned.npy', Y_train_det_list_new)
