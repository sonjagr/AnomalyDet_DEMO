...
if use_ae:
    ...
    ae.load(os.path.join(base_dir, 'checkpoints/TQ3_1_TQ3_more_data/AE_TQ3_510_to_510_epochs'))
...
train_ds_normal_l = train_ds_normal_all.filter(lambda x, y: tf.reduce_mean(x) > mean_error).cache()
...
train_ds_normal_extra = train_ds_normal_all.concatenate(train_ds_normal_l_rotated).shuffle(1.5*normal, reshuffle_each_iteration=True)

train_ds_normal_sample = train_ds_normal_extra.take(augmented)
