...
DataBaseFile = 'extra_annotaions_from_normals'
imgDir = imgDir_pc
...
if load_from_file_list:
    X_test_list_ae_toann = np.load(os.path.join(base_dir, dir_ae, 'X_test_forcnn_toann.npy'), allow_pickle=True)
    ...
    list_to_annotate = np.append(X_train_list_ae_toann, X_test_list_ae_toann)

    all_files_db = []
    empty_list = np.array([]).tolist()
    for i in list_to_annotate:
        _campaign, _dut, _fn = i.split('/')
