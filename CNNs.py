...
model_simple = tf.keras.Sequential([
    tf.keras.layers.InputLayer(input_shape=(160,160, 1)),
    tf.keras.layers.experimental.preprocessing.Rescaling(1. / 243.),
    tf.keras.layers.Conv2D(filters=16, kernel_size=(20,20), strides=(20,20), padding="valid",activation='relu', use_bias=True),
    tf.keras.layers.Dropout(0.3),
    tf.keras.layers.Conv2D(filters=16, kernel_size=(8, 8), strides=(8, 8),  padding="valid", activation='relu', use_bias=True),
    tf.keras.layers.Dropout(0.2),
