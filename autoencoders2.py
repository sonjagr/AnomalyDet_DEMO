...
class AutoEncoder(tf.keras.Model):
    ...
    def initialize_network_TQ3_DO2(self):
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(
                    2720, PICTURESIZE_X, 1)),
    ...
    def initialize_network_SG3(self):
        ...
        self.decoder = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dropout(0.15),
                tf.keras.layers.Conv2DTranspose(
                    filters=1, kernel_size=(8, 10), strides=(6, 5), activation='elu', padding="valid",
                    use_bias=True),
                tf.keras.layers.Conv2D(
    ...
    def initialize_network_SG3_DO(self):
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(
                    2720, PICTURESIZE_X, 1)),
    ...
    def initialize_network_SG3_DO2(self):
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(
                    2720, PICTURESIZE_X, 1)),
