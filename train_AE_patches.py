...
from common import *
from helpers.cnn_helpers import patch_images_AE, crop_AE, format_data_AE
...
train_dataset = create_dataset(X_train_list, _shuffle=True)
test_dataset = create_dataset(X_test_list, _shuffle=True)
val_dataset = create_dataset(X_val_list, _shuffle=True)

train_dataset = train_dataset.map(lambda item: tuple(tf.py_function(crop_AE, [item], [tf.float32,])))
test_dataset = test_dataset.map(lambda item: tuple(tf.py_function(crop_AE, [item], [tf.float32,])))

train_dataset = train_dataset.flat_map(patch_images_AE).unbatch()
test_dataset = test_dataset.flat_map(patch_images_AE).unbatch()

train_dataset = train_dataset.map(format_data_AE).cache().shuffle(buffer_size=1000*408, reshuffle_each_iteration=True).batch(batch_size)
test_dataset = test_dataset.map(format_data_AE).cache().batch(batch_size)
...
ae = AutoEncoder()
if load == False:
    if model_ID == 'TQ3_3_patches':
        ae.initialize_network_TQ3_3_patches()
    else:
...
if load == True:
    ae.load(cp_loc + model_ID + '_' + str(3264) + '_' + str(savename) + '/AE_' + model_ID + '_' + str(cont_epoch) + '_to_' + str(cont_epoch) + '_epochs')
    with open(cp_loc + model_ID + '_' + str(3264) + '_' + str(savename) + '/cost.pkl', "rb") as cost_file_pkl:
